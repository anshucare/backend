...
function applySiteContent() {
    ...
    if (siteContent.contact.whatsapp) {
        const waNum = siteContent.contact.whatsapp.replace(/\+/g, '').replace(/\s/g, '');
        if (waFloat) waFloat.href = `https://wa.me/${waNum}`;

        // Update Modal Link if exists
        const modalLink = document.querySelector('.chat-modal-body a[href*="wa.me"]');
        if (modalLink) modalLink.href = `https://wa.me/${waNum}`;
    }
